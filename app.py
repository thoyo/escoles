...
def find_features(lat, lng, radius=500):
    ...
    edu_features = sorted(edu_features, key=lambda x: x[1])[:3]  # 3 nearest where titularitat = "Departament d'Educació"
    non_edu_features = sorted(non_edu_features, key=lambda x: x[1])[:3]  # 3 nearest where titularitat != "Departament d'Educació"

    # Log reasons for the nearest features
    for feature, distance in edu_features:
    ...
    for feature, distance in non_edu_features:
    ...
    edu_features = [item[0] for item in edu_features]
    non_edu_features = [item[0] for item in non_edu_features]

    # Combine all unique features (no duplicates)
    all_features = {f["properties"]["codi_centre"]: f for f in (nearby_features + edu_features + non_edu_features + area_features)}
